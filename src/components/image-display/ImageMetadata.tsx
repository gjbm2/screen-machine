...
const ImageMetadata: React.FC<ImageMetadataProps> = ({ 
  ...
  return (
    <div className="flex justify-between items-center text-2xs xs:text-xs md:text-sm text-muted-foreground">
      ...
      <div className="flex items-center gap-1 xs:gap-2 md:gap-4">
        ...
        {imageUrl && onOpenInNewTab && (
          <Button 
            variant="outline" 
            size="sm" 
            className="h-5 md:h-7 px-1 md:px-2 text-2xs xs:text-xs flex items-center gap-1 text-muted-foreground whitespace-nowrap"
            onClick={onOpenInNewTab}
            aria-label="Open image in new tab"
            onMouseDown={(e) => e.preventDefault()} // Prevent text selection
          >
            <ExternalLink className="h-2.5 w-2.5 md:h-3.5 md:w-3.5" />
